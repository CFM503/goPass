...
- **本版为稳定版**：自 v1.8 系列起累计推进的稳定性、安全、性能与清理修复至此全部收口——五个批次（稳定/安全/性能/清理/测试）的台账条目清零，并补上引擎关键路径的回归测试：`internal/engine` 语句覆盖率由 32.5% 升至 48.1%，`go build`、`go vet`、`go test ./...` 全绿，CI 另行执行 `-race`。定位为可长期驻留的稳定版本。
...
- **Web 控制台进站超时**：`StartServer` 建的 `*http.Server` 四个超时字段全是默认的 0（无限），等于把连接的生命周期整个交给对端——没有 `ReadHeaderTimeout` 挡不住 Slowloris（连接开着、请求头逐字节送，goroutine 与 socket 无限期挂着，Go 文档点名要这个字段来防），没有 `ReadTimeout` 让请求体可以无限滴灌、handler 卡在 `Decode` 上不回来，没有 `IdleTimeout` 让 keep-alive 连接永不回收。现在进站三项设为 5s/15s/60s，且只约束"读"：浏览器的请求头和几百字节的 JSON 体都是瞬间写完的，状态轮询每 3 秒一次远够不到空闲上限，即便标签页进后台导致轮询暂停、连接被回收，下次 `fetch` 也只是悄悄新开一条，页面无感知。`WriteTimeout` 刻意保持 0——它约束"handler 执行 + 回包"的总时长，设了就等于允许把响应拦腰切断，磁盘卡顿时会出现配置已写成功、界面却报"保存失败"的矛盾，而本服务响应只有几百字节、内核缓冲区直接收下，慢读的客户端占不到什么便宜。
