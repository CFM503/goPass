...
func isProcessElevated()bool{token,err:=windows.OpenProcessToken(windows.CurrentProcess(),windows.TOKEN_QUERY);if err!=nil{return false};defer token.Close();var elevation windows.TokenElevation;var size uint32;err=windows.GetTokenInformation(token,windows.TokenElevation,(*byte)(unsafe.Pointer(&elevation)),uint32(unsafe.Sizeof(elevation)),&size);return err==nil&&elevation.TokenIsElevated!=0}
...
func(i *Interceptor)Start()error{if err:=i.preflight();err!=nil{return err};h,err:=wdOpen(i.buildFilter(),layerNetwork,priorityDefault,0);if err!=nil{return fmt.Errorf("WinDivert 启动失败：%w",err)};i.mu.Lock();i.handle=h;i.mu.Unlock();_=i.resolver.Refresh();log.Println("[WinDivert] process whitelist TCP interception ready");go i.refreshLoop();buf:=make([]byte,65535);for{select{case<-i.stopCh:return nil;default:};i.mu.RLock();h=i.handle;i.mu.RUnlock();if h==nil{return nil};n,addr,err:=h.Recv(buf);if err!=nil{select{case<-i.stopCh:return nil;default:time.Sleep(5*time.Millisecond)};continue};if n>0&&n<=len(buf){i.handlePacket(buf[:n],addr)}}}
