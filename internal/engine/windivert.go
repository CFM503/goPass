...
func(i *Interceptor)buildFilter()string{i.mu.RLock();proxyIP:=i.proxyIP;tproxyPort:=i.tproxyPort;i.mu.RUnlock();f:="(outbound and ip and tcp and ip.DstAddr != 127.0.0.1";if pip:=net.ParseIP(proxyIP);pip!=nil&&pip.To4()!=nil&&!pip.IsLoopback(){f+=fmt.Sprintf(" and ip.DstAddr != %s",pip.To4().String())};f+=")";return fmt.Sprintf("%s or (outbound and ip and tcp and tcp.SrcPort == %d)",f,tproxyPort)}
func(i *Interceptor)Start(){h,err:=wdOpen(i.buildFilter(),layerNetwork,priorityDefault,0);if err!=nil{log.Printf("[WinDivert] start failed: %v",err);return};i.mu.Lock();i.handle=h;i.mu.Unlock();_=i.resolver.Refresh();log.Println("[WinDivert] process whitelist TCP interception ready");go i.refreshLoop();buf:=make([]byte,65535);for{select{case<-i.stopCh:return;default:};i.mu.RLock();h=i.handle;i.mu.RUnlock();if h==nil{return};n,addr,err:=h.Recv(buf);if err!=nil{select{case<-i.stopCh:return;default:time.Sleep(5*time.Millisecond)};continue};if n>0&&n<=len(buf){i.handlePacket(buf[:n],addr)}}}
func(i *Interceptor)refreshLoop(){ticker:=time.NewTicker(250*time.Millisecond);defer ticker.Stop();for{select{case<-i.stopCh:return;case<-ticker.C:_=i.resolver.Refresh()}}}
func(i *Interceptor)handlePacket(pkt []byte,addr *winDivertAddress){if len(pkt)<20||(pkt[0]>>4)!=4{return};ihl:=int(pkt[0]&0x0f)*4;if ihl<20||len(pkt)<ihl+4||pkt[9]!=6{return};srcIP:=net.IPv4(pkt[12],pkt[13],pkt[14],pkt[15]);dstIP:=net.IPv4(pkt[16],pkt[17],pkt[18],pkt[19]);srcPort:=binary.BigEndian.Uint16(pkt[ihl:ihl+2]);dstPort:=binary.BigEndian.Uint16(pkt[ihl+2:ihl+4]);i.mu.RLock();proxyIP:=i.proxyIP;tproxyPort:=i.tproxyPort;i.mu.RUnlock();if srcPort==tproxyPort{i.reverseNAT(pkt,addr,ihl);return};if proxyIP!=""{if pip:=net.ParseIP(proxyIP);pip!=nil&&pip.Equal(dstIP){i.sendPass(pkt,addr);return}};if isPrivateOrLocal(dstIP){i.sendPass(pkt,addr);return};flow:=process.Flow{LocalIP:binary.LittleEndian.Uint32(pkt[12:16]),LocalPort:srcPort,RemoteIP:binary.LittleEndian.Uint32(pkt[16:20]),RemotePort:dstPort};entry,ok:=i.resolver.Lookup(flow);if !ok||!i.isWhitelisted(entry.Name){i.sendPass(pkt,addr);return};i.hijackTCP(pkt,addr,srcIP,dstIP,srcPort,dstPort,ihl)}
...
func(i *Interceptor)hijackTCP(pkt []byte,addr *winDivertAddress,origSrcIP,origDstIP net.IP,srcPort,dstPort uint16,ihl int){i.tracker.Set("127.0.0.1",srcPort,origSrcIP,srcPort,origDstIP,dstPort,addr.IfIdx,addr.SubIfIdx);loop:=net.IPv4(127,0,0,1).To4();copy(pkt[12:16],loop);copy(pkt[16:20],loop);i.mu.RLock();tp:=i.tproxyPort;i.mu.RUnlock();binary.BigEndian.PutUint16(pkt[ihl+2:ihl+4],tp);addr.Bits|=flagOutbound|flagLoopback;if err:=i.calcAndSend(pkt,addr);err!=nil{log.Printf("[WinDivert] hijack send failed: %v",err)}}
