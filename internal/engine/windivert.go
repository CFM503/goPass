...
// YouTube compatibility follows the known-good early implementation:
// capture outbound IPv6 and drop it so dual-stack connections cannot bypass the IPv4 TProxy;
// leave UDP/443 completely alone so Windows/Chromium can perform its normal QUIC failure/fallback.
func(i *Interceptor)buildFilter()string{i.mu.RLock();proxyIP:=i.proxyIP;tproxyPort:=i.tproxyPort;i.mu.RUnlock();f:="(outbound and ip and tcp and ip.DstAddr != 127.0.0.1";if pip:=net.ParseIP(proxyIP);pip!=nil&&pip.To4()!=nil&&!pip.IsLoopback(){f+=fmt.Sprintf(" and ip.DstAddr != %s",pip.To4().String())};f+=")";return fmt.Sprintf("%s or (outbound and ipv6) or (outbound and ip and tcp and tcp.SrcPort == %d)",f,tproxyPort)}
