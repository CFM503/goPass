...
type Engine struct{cfg *config.Config;cfgMu sync.RWMutex;tracker *ConnTracker;tproxy *TProxy;interceptor *Interceptor;controller *controller.RouteController;Stats *Stats}
...
func New(cfg *config.Config)(*Engine,error){stats:=&Stats{PID:os.Getpid(),active:make(map[string]map[string]interface{})};return &Engine{cfg:cfg,tracker:NewConnTracker(cfg.System.ConnTrackGCInterval,cfg.System.ConnTrackTTL),Stats:stats},nil}
func(e *Engine)Controller()*controller.RouteController{return nil}
func(e *Engine)Start()error{log.Printf("[Engine] GoPass v1.6.7 starting, PID=%d",e.Stats.PID);proxyAddr,proxyType:="","";for _,srv:=range e.cfg.Outbounds.Servers{if srv.Type=="socks5"||srv.Type=="http"{proxyAddr=fmt.Sprintf("%s:%d",srv.Address,srv.Port);proxyType=srv.Type;break}};if proxyAddr==""{return fmt.Errorf("no socks5 or http upstream configured")};proxyHost,proxyPort,err:=parseAddr(proxyAddr);if err!=nil{return fmt.Errorf("invalid upstream address: %w",err)};tp,err:=NewTProxy(e.tracker,proxyType,proxyAddr,e.Stats,e.cfg.Performance,e.cfg.System.TProxyPort);if err!=nil{return err};e.tproxy=tp;go tp.Accept();i:=NewInterceptor(e.tracker,proxyFilterIP(proxyHost),uint16(proxyPort),uint16(e.cfg.System.TProxyPort),e.cfg.ProcessWhitelist);e.interceptor=i;go i.Start();log.Printf("[Engine] ready: upstream=%s tproxy=%d whitelist=%v",proxyAddr,e.cfg.System.TProxyPort,e.cfg.ProcessWhitelist);return nil}
...
func(e *Engine)Stop(){if e.interceptor!=nil{e.interceptor.Close()};if e.tproxy!=nil{_=e.tproxy.Close()};if e.tracker!=nil{e.tracker.Close()};CleanUpOnShutdown()}
