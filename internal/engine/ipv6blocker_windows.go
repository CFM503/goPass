...
func (b *IPv6Blocker) preflight() error {
    ...
    if err != nil { return fmt.Errorf("WinDivert IPv6 blocker 初始化失败：%w", err) }
    h.Close()
    return nil
}
...
func (b *IPv6Blocker) Start() error {
    h, err := wdOpen(b.buildFilter(), layerNetwork, priorityDefault, 0)
    if err != nil { return fmt.Errorf("WinDivert IPv6 blocker 启动失败：%w", err) }
    b.mu.Lock(); b.handle = h; b.mu.Unlock()
    ...
    buf := make([]byte, 65535)
    for {
        select { case <-b.stopCh: return nil; default: }
        b.mu.RLock(); h = b.handle; b.mu.RUnlock()
        if h == nil { return nil }
        n, addr, err := h.Recv(buf)
        if err != nil {
            select { case <-b.stopCh: return nil; default: time.Sleep(5 * time.Millisecond) }
            continue
        }
        if n > 0 && n <= len(buf) { b.handlePacket(buf[:n], addr) }
    }
}
...
func (b *IPv6Blocker) refreshLoop() {
    ...
    for {
        ...
        case <-ticker.C: process.RefreshIPv6TCP()
        }
    }
}

func (b *IPv6Blocker) handlePacket(pkt []byte, addr *winDivertAddress) {
    if len(pkt) < 40 || (pkt[0]>>4) != 6 { b.sendPass(pkt, addr); return }
    next := pkt[6]
    if next != 6 { b.sendPass(pkt, addr); return }
    if len(pkt) < 60 { b.sendPass(pkt, addr); return }
    srcPort := binary.BigEndian.Uint16(pkt[40:42])
    dstPort := binary.BigEndian.Uint16(pkt[42:44])
    if b.isWhitelistedTCP(pkt[8:24], srcPort, pkt[24:40], dstPort) { return }
    b.sendPass(pkt, addr)
}

func (b *IPv6Blocker) isWhitelistedTCP(src, dst []byte, srcPort, dstPort uint16) bool {
    if len(src) != 16 || len(dst) != 16 { return false }
    var srcIP, dstIP [16]byte
    copy(srcIP[:], src); copy(dstIP[:], dst)
...
func (b *IPv6Blocker) isWhitelisted(name string) bool {
    b.mu.RLock(); _, ok := b.whitelist[strings.ToLower(name)]; b.mu.RUnlock(); return ok
}
...
func (b *IPv6Blocker) Close() {
    b.closeOnce.Do(func() {
        close(b.stopCh)
        b.mu.Lock(); h := b.handle; b.handle = nil; b.mu.Unlock()
        if h != nil { h.Close() }
    })
}
