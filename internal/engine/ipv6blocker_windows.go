...
func (b *IPv6Blocker) handlePacket(pkt []byte, addr *winDivertAddress) {
	...
	if next == 6 {
		...
		// Keep argument order explicit: source IP, destination IP, source port, destination port.
		if b.isWhitelistedTCP(pkt[8:24], pkt[24:40], srcPort, dstPort) {
			return
		}
		b.sendPass(pkt, addr)
		return
	...
	if next == 17 {
		...
		var localIP [16]byte
		copy(localIP[:], pkt[8:24])
		srcPort := binary.BigEndian.Uint16(pkt[40:42])
		name, ok := process.LookupIPv6UDP(process.IPv6UDPFlow{LocalIP: localIP, LocalPort: srcPort})
		if ok && b.isWhitelisted(name) {
			return
		}
		b.sendPass(pkt, addr)
		return
...
func (b *IPv6Blocker) isWhitelistedTCP(src, dst []byte, srcPort, dstPort uint16) bool {
	...
	copy(srcIP[:], src)
	copy(dstIP[:], dst)
	name, ok := process.LookupIPv6TCP(process.IPv6TCPFlow{
		LocalIP:    srcIP,
		LocalPort:  srcPort,
		RemoteIP:   dstIP,
		RemotePort: dstPort,
