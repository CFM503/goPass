...
func (ct *ConnTracker) Set(mappedIP string, mappedPort uint16, origSrcIP net.IP, origSrcPort uint16, origDstIP net.IP, origDstPort uint16, origIfIdx, origSubIfIdx uint32) {
	ct.mu.Lock()
...
func (ct *ConnTracker) Get(srcIP string, srcPort uint16) (ConnTarget, bool) {
	ct.mu.RLock(); v, ok := ct.entries[ConnKey{srcIP, srcPort}]; ct.mu.RUnlock()
	return v, ok
}

func (ct *ConnTracker) Delete(srcIP string, srcPort uint16) {
	ct.mu.Lock(); delete(ct.entries, ConnKey{srcIP, srcPort}); ct.mu.Unlock()
}
...
func (ct *ConnTracker) Close() {
	ct.closeOnce.Do(func() { close(ct.stopCh) })
}
