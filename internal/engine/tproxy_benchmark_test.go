...
func runProxyBufferBenchmark(b *testing.B, bufSize int) {
	...
	go func() {
		defer close(done)
		for i := 0; i < b.N; i++ { if _, err := srcW.Write(data); err != nil { return } }
		_ = srcW.Close()
	}()

	b.ResetTimer()
	buf := getRelayBuffer(bufSize)
	...
	for {
		...
		if n > 0 {
			...
			if _, werr := dstW.Write(buf[:n]); werr != nil { break }
		}
		if err != nil { break }
	}
	b.StopTimer()
	_ = srcR.Close(); _ = dstW.Close(); _ = dstR.Close()
	<-done
}
