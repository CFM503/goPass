...
func (u *UpstreamDialer) Update(pType, pAddr string) { u.mu.Lock(); u.pType, u.pAddr, u.cached, u.cachedKey = pType, pAddr, nil, ""; u.mu.Unlock() }
...
func (u *UpstreamDialer) Dialer() (proxy.Dialer, error) {
	...
	var d proxy.Dialer;var err error
	if pType=="http"{d,err=NewHTTPProxy(pAddr,"","",forward)}else{d,err=proxy.SOCKS5("tcp",pAddr,nil,forward)}
