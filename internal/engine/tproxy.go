...
func originalTarget(ip net.IP, port uint16) (string, error) {
	if ip == nil || ip.IsUnspecified() {
		return "", fmt.Errorf("invalid original destination IP")
	}
	if port == 0 {
		return "", fmt.Errorf("invalid original destination port")
	}
...
func (tp *TProxy) UpdateUpstream(pType, pAddr string) {
	...
	log.Printf("[TProxy] upstream proxy switched to %s -> %s", pType, pAddr)
}
...
func (tp *TProxy) handleConn(conn net.Conn) {
	...
	tune := func(c net.Conn) {
		if tc, ok := c.(*net.TCPConn); ok {
			...
			if keepAlivePeriod > 0 { _ = tc.SetKeepAlivePeriod(time.Duration(keepAlivePeriod) * time.Second) }
			if tcpLinger >= 0 { _ = tc.SetLinger(tcpLinger) }
		}
	}
	tune(conn)
	tune(remote)
	...
	if tp.stats != nil {
		tp.stats.AddActiveConn(map[string]interface{}{
			"id": fmt.Sprintf("%s:%d", srcIP, srcPort),
			"target": targetAddr,
		})
		defer tp.stats.RemoveActiveConn(fmt.Sprintf("%s:%d", srcIP, srcPort))
	}
	...
	go func() {
		defer wg.Done()
		result <- tp.relay(remote, conn, false, bufSize)
	}()
	go func() {
		defer wg.Done()
		result <- tp.relay(conn, remote, true, bufSize)
	}()
