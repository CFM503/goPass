	...
	tune := func(c net.Conn) {
		if tc, ok := c.(*net.TCPConn); ok {
			...
			if keepAlivePeriod > 0 {
				_ = tc.SetKeepAlivePeriod(time.Duration(keepAlivePeriod) * time.Second)
			}
			if tcpLinger >= 0 {
				_ = tc.SetLinger(tcpLinger)
			}
		}
	}
	tune(conn)
...
func (w writerConn) Write(p []byte) (int, error) {
	...
	if n > 0 && w.stats != nil {
		if w.tx {
			atomic.AddInt64(&w.stats.TxBytes, int64(n))
		} else {
			atomic.AddInt64(&w.stats.RxBytes, int64(n))
		}
	}
	return n, err
}
