...
var relayBufferPool = sync.Pool{
	New: func() any { return make([]byte, defaultRelayBufferSize) },
}
...
func NewTProxy(tracker *ConnTracker, proxyType, proxyAddr string, stats *Stats, perf config.PerformanceConfig, tproxyPort int) (*TProxy,error) {
	ln,err:=net.Listen("tcp",fmt.Sprintf("0.0.0.0:%d",tproxyPort));if err!=nil{return nil,fmt.Errorf("TProxy listen failed: %w",err)}
	return &TProxy{listener:ln,tracker:tracker,upstream:NewUpstreamDialer(proxyType,proxyAddr),bufferSize:config.ClampBufferSize(perf.BufferSize),tcpNoDelay:perf.TCPNoDelay,tcpKeepAlive:perf.TCPKeepAlive,keepAlivePeriod:perf.KeepAlivePeriod,tcpLinger:perf.TCPLinger,stats:stats},nil
}
