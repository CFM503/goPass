...
func DefaultAutomaticRouteConfig() AutomaticRouteConfig {
	return AutomaticRouteConfig{Enabled:false,CheckInterval:5,StandbyCheckInterval:30,RecoverCheckInterval:120,SwitchThreshold:5.0,SwitchCooldown:60,MinimumStableTime:30,FailureThreshold:3,RecoveryThreshold:3,PeakMode:"auto",PeakStartHour:18,PeakEndHour:23,HistoryWindow:60,StandbyCount:3,MaxProbeConcurrency:4,HistoryFile:"routes_history.json",HistorySaveInterval:300,Routes:[]RouteConfig{{ID:"goway-default",Name:"GOWAY Default",Address:"127.0.0.1",Port:9192,Protocol:"socks5",Type:"goway"}}}
}

func NormalizeAutomaticRoute(a AutomaticRouteConfig) AutomaticRouteConfig {
	def:=DefaultAutomaticRouteConfig()
	if a.CheckInterval<=0{a.CheckInterval=def.CheckInterval};if a.StandbyCheckInterval<=0{a.StandbyCheckInterval=def.StandbyCheckInterval};if a.RecoverCheckInterval<=0{a.RecoverCheckInterval=def.RecoverCheckInterval};if a.SwitchThreshold<=0{a.SwitchThreshold=def.SwitchThreshold};if a.SwitchCooldown==0{a.SwitchCooldown=def.SwitchCooldown}else if a.SwitchCooldown<0{a.SwitchCooldown=0};if a.MinimumStableTime==0{a.MinimumStableTime=def.MinimumStableTime}else if a.MinimumStableTime<0{a.MinimumStableTime=0};if a.FailureThreshold<=0{a.FailureThreshold=def.FailureThreshold};if a.RecoveryThreshold<=0{a.RecoveryThreshold=def.RecoveryThreshold};if a.PeakMode==""{a.PeakMode=def.PeakMode};if a.PeakStartHour==0&&a.PeakEndHour==0{a.PeakStartHour=def.PeakStartHour;a.PeakEndHour=def.PeakEndHour};if a.HistoryWindow<=0{a.HistoryWindow=def.HistoryWindow};if a.StandbyCount<=0{a.StandbyCount=def.StandbyCount};if a.MaxProbeConcurrency<=0{a.MaxProbeConcurrency=def.MaxProbeConcurrency};if a.HistoryFile==""{a.HistoryFile=def.HistoryFile};if a.HistorySaveInterval<=0{a.HistorySaveInterval=def.HistorySaveInterval};if len(a.Routes)==0{a.Routes=def.Routes};return a
}

func DefaultConfig()*Config{return &Config{API:APIConfig{ListenAddr:"127.0.0.1:8080",WSRefreshInterval:5,UIConnLimit:20},Outbounds:OutboundConfig{Servers:[]Server{{Tag:"proxy",Type:"socks5",Address:"127.0.0.1",Port:9192}}},Performance:PerformanceConfig{BufferSize:256*1024,TCPNoDelay:true,TCPKeepAlive:true,KeepAlivePeriod:15,TCPLinger:-1},System:SystemConfig{TProxyPort:7893,ConnTrackGCInterval:15,ConnTrackTTL:30},AutomaticRoute:DefaultAutomaticRouteConfig()}}

func(c *Config)UnmarshalJSON(data []byte)error{type Alias Config;aux:=&struct{*Alias;OldOutbound *OutboundConfig `json:"outbound"`}{Alias:(*Alias)(c)};if err:=json.Unmarshal(data,&aux);err!=nil{return err};if aux.OldOutbound!=nil&&len(c.Outbounds.Servers)==0{c.Outbounds=*aux.OldOutbound};def:=DefaultConfig();if c.API.ListenAddr==""{c.API.ListenAddr=def.API.ListenAddr};if c.API.WSRefreshInterval<=0{c.API.WSRefreshInterval=def.API.WSRefreshInterval};if c.API.UIConnLimit<=0{c.API.UIConnLimit=def.API.UIConnLimit};if c.System.TProxyPort==0{c.System=def.System}else{if c.System.ConnTrackGCInterval<=0{c.System.ConnTrackGCInterval=def.System.ConnTrackGCInterval};if c.System.ConnTrackTTL<=0{c.System.ConnTrackTTL=def.System.ConnTrackTTL}};if c.Performance.BufferSize==0{c.Performance=def.Performance}else{c.Performance.BufferSize=ClampBufferSize(c.Performance.BufferSize)};c.AutomaticRoute=NormalizeAutomaticRoute(c.AutomaticRoute);return nil}
