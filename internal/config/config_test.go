...
func TestClampBufferSize(t *testing.T) {
	tests := []struct{ input, expected int }{
		{0, 32 * 1024}, {-1, 32 * 1024}, {32767, 32768}, {32768, 32768},
		{256 * 1024, 256 * 1024}, {512 * 1024, 512 * 1024}, {1024 * 1024, 1024 * 1024},
		{1024*1024 + 1, 1024 * 1024}, {2 * 1024 * 1024, 1024 * 1024},
	}
	for _, tt := range tests {
		if got := ClampBufferSize(tt.input); got != tt.expected { t.Errorf("ClampBufferSize(%d) = %d, want %d", tt.input, got, tt.expected) }
	}
}
...
func TestAutomaticRouteConfigDefaults(t *testing.T) {
	c := DefaultConfig()
	if c.AutomaticRoute.CheckInterval != 5 { t.Errorf("CheckInterval = %d, want 5", c.AutomaticRoute.CheckInterval) }
	if c.AutomaticRoute.SwitchThreshold != 5.0 { t.Errorf("SwitchThreshold = %f, want 5.0", c.AutomaticRoute.SwitchThreshold) }
	if c.AutomaticRoute.FailureThreshold != 3 { t.Errorf("FailureThreshold = %d, want 3", c.AutomaticRoute.FailureThreshold) }
	if c.AutomaticRoute.RecoveryThreshold != 3 { t.Errorf("RecoveryThreshold = %d, want 3", c.AutomaticRoute.RecoveryThreshold) }
	if len(c.AutomaticRoute.Routes) == 0 { t.Error("default should contain one route") }
}
