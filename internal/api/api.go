...
func (s *Server) handleUpstream(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err == nil && s.engine != nil {
...
func (s *Server) handleRoutesReport(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type","application/json")
	if r.Method != http.MethodPost || s.engine == nil || s.engine.Controller() == nil { http.Error(w,"controller not ready",503); return }
	var raw json.RawMessage
	if err := json.NewDecoder(r.Body).Decode(&raw); err != nil { http.Error(w,"invalid json",400); return }
	var reports []controller.RouteMetricReport
	if err := json.Unmarshal(raw,&reports); err != nil { var one controller.RouteMetricReport; if err2:=json.Unmarshal(raw,&one); err2!=nil { http.Error(w,"invalid payload",400); return }; reports=[]controller.RouteMetricReport{one} }
