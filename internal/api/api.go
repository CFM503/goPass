...
func (s *Server) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	if s.engine != nil && s.engine.Stats != nil {
		pid, conns = s.engine.Stats.PID, s.engine.Stats.Connections
	}
...
func (s *Server) handleSettings(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		if s.engine != nil {
			...
			interval, connLimit = cfg.API.WSRefreshInterval, cfg.API.UIConnLimit
		}
	...
	if s.engine != nil {
		...
		_ = s.engine.SaveConfig("config.json")
	}
...
func (s *Server) handleUpstream(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		if s.engine != nil {
			cfg := s.engine.GetConfig()
			if len(cfg.Outbounds.Servers) > 0 {
				srv := cfg.Outbounds.Servers[0]
				pType, pAddr, pPort = srv.Type, srv.Address, srv.Port
			}
		}
		_ = json.NewEncoder(w).Encode(map[string]interface{}{"type": pType, "address": pAddr, "port": pPort})
		return
	}
	if r.Method == http.MethodPost {
		var req struct {
...
func (s *Server) handlePerformance(w http.ResponseWriter, r *http.Request) {
	...
	if perf.BufferSize == 0 {
		perf.BufferSize = config.DefaultConfig().Performance.BufferSize
	} else {
		perf.BufferSize = config.ClampBufferSize(perf.BufferSize)
	}
	if perf.KeepAlivePeriod < 1 || perf.KeepAlivePeriod > 3600 {
		perf.KeepAlivePeriod = 15
	}
	perf.TCPLinger = -1
	if s.engine != nil {
...
func (s *Server) handleRoutes(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if err := ctrl.RegisterRoute(controller.NewRoute(req.ID, req.Name, req.Address, req.Port, req.Protocol, req.Type)); err != nil {
			_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "error", "error": err.Error()})
			return
		}
		_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "ok"})
...
func (s *Server) handleRoutesCurrent(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	if s.engine == nil || s.engine.Controller() == nil {
		http.Error(w, "controller not ready", 503)
		return
	}
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"current": s.engine.Controller().GetCurrentRoute()})
}
...
func (s *Server) handleRoutesStandby(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"standby": s.engine.Controller().GetStandbyRoutes()})
}
...
func (s *Server) handleRoutesMetrics(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"metrics": s.engine.Controller().GetMetrics()})
}

func (s *Server) handleRoutesSwitch(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	if r.Method != http.MethodPost || s.engine == nil || s.engine.Controller() == nil {
		http.Error(w, "controller not ready", 503)
		return
	}
	var req struct{ ID string `json:"id"` }
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "ok", "current": s.engine.Controller().GetCurrentRoute()})
}
...
func (s *Server) handleRoutesEnable(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "ok", "auto_enabled": true})
}
...
func (s *Server) handleRoutesDisable(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "ok", "auto_enabled": false})
}
...
func (s *Server) handleRoutesReport(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(raw, &reports); err != nil {
		var one controller.RouteMetricReport
		if err2 := json.Unmarshal(raw, &one); err2 != nil {
			http.Error(w, "invalid payload", 400)
			return
		}
		reports = []controller.RouteMetricReport{one}
	}
	s.engine.Controller().ReportMetrics(reports)
	_ = json.NewEncoder(w).Encode(map[string]interface{}{"status": "ok", "ingested": len(reports)})
}
